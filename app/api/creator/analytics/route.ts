...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Ownership check: a creator can only read their own analytics.
    ...
    try {
      ...
      // Fallback if creatorIds index is not available
      ordersSnapshot = await adminDb
        .collection("orders")
        .where("createdAt", ">=", startDate)
        .where("createdAt", "<=", endDate)
    ...
    const orders = ordersSnapshot.docs
      .filter((doc: any) => {
        const data = doc.data()
        // Ensure this order actually contains items for this creator
    ...
    // Calculate metrics using creator-specific revenue
    let totalRevenue = 0
    let totalOrders = 0
    ...
    // Previous period metrics (creator-specific)
    let prevRevenue = 0
    ...
    // Load store page views from creatorstats collection
    let storeViews = 0
    try {
      const creatorstatsDoc = await adminDb
        .collection("creatorstats")
        .doc(creatorId)
        .get()

      if (creatorstatsDoc.exists) {
        const data = creatorstatsDoc.data()
        storeViews = data?.storeViews || 0
      }
    } catch (error) {
      console.error("Error loading creatorstats for store views:", error)
    }
    ...
    // Build sales data by date (creator-specific revenue)
    ...
    // Get top products by revenue (only this creator's items)
    ...
    // Recent orders (with creator-specific totals)
    const recentOrders = orders
      ...
      .map((order: any) => ({
        ...
        total: calculatecreatorOrderRevenue(order),
