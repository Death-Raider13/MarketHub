  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isGoogleUser && user) {
        // Google-authenticated vendor: just create the store document
        const storeData = {
        ...
        // Vendors collection doc id must match auth uid for rules
        await setDoc(doc(db, "vendors", user.uid), storeData, { merge: true })

        // Notify applicant and admins (best-effort)
        try {
          await fetch('/api/notifications/vendor-application', {
            method: 'POST',
      ...
      // Email/password path: create account then proceed as before
      ...
      // TODO: Save to Firestore with user UID once available
