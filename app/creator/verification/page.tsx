...
function VerificationContent() {
  ...
  return <div className="flex min-h-screen flex-col"><Header /><main className="flex-1 bg-muted/30"><div className="container mx-auto max-w-5xl px-4 py-10">
    ...
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><UploadCloud className="h-5 w-5 text-primary" />Additional Resource Uploads</CardTitle><CardDescription>Upload your first three books, educational resources, or videos at no cost.</CardDescription></CardHeader><CardContent className="space-y-5"><p className="text-3xl font-bold">₦{uploadFee.toLocaleString()}</p><p className="text-sm text-muted-foreground">Pay now to unlock upload access beyond the three free resources. You can purchase this before using your free uploads, and the access will remain available for future uploads. {waitlistEligible ? "Your waitlist discount is applied." : "Eligible waitlist members may receive the discounted rate."}</p><div className="rounded-lg border bg-muted/40 p-3 text-sm">{resourceCount < 3 ? <>You still have <strong>{3 - resourceCount} free resource{3 - resourceCount === 1 ? "" : "s"}</strong> available.</> : <>Your three free resources have been used.</>}</div><Button className="w-full" variant="outline" onClick={() => startPayment("uploads")} disabled={paying !== null}>{paying === "uploads" ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <UploadCloud className="mr-2 h-4 w-4" />}Pay ₦{uploadFee.toLocaleString()} to unlock more uploads</Button></CardContent></Card>
    </div>
