...
function CreatorServicesContent() {
  const { user } = useAuth()
  ...
  const loadServiceBookings = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/creator/services?creatorId=${user?.uid}`)
  ...
  const scheduleService = async (bookingId: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/creator/services/${bookingId}/schedule`, {
        method: 'POST',
        headers: {
  ...
  const updateServiceStatus = async (bookingId: string, status: string, notes?: string) => {
    try {
      ...
      const response = await fetch(`/api/creator/services/${bookingId}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
                      ...
                      <Dialog>
                        ...
                        <DialogContent className="max-w-2xl">
                          ...
                          <div className="space-y-4">
                              ...
                              <div>
                                ...
                                <Select value={scheduleForm.location} onValueChange={(value) => setScheduleForm({...scheduleForm, location: value})}>
                                  ...
                                  <SelectContent>
                                    ...
                                    <SelectItem value="creator_location">My Location</SelectItem>
                            ...
                            <div>
                              <Label htmlFor="creatorNotes">Notes for Customer</Label>
                              <Textarea
                                id="creatorNotes"
                                value={scheduleForm.creatorNotes}
                                onChange={(e) => setScheduleForm({...scheduleForm, creatorNotes: e.target.value})}
