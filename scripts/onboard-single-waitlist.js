...
async function onboardSingleEmail(targetEmail) {
  ...
  console.log(`\n🚀 Retrying onboarding for single email: ${email}`);
  ...
  try {
    ...
    console.log(`   ✓ Updated Firestore users document with waitlistMember=true.`);

    // Generate Password Reset / Account Activation Link
    const passwordResetLink = await auth.generatePasswordResetLink(email);
    console.log(`   🔑 Password setup link generated: ${passwordResetLink}`);

    const roleName = 'Student & Reader';
    ...
    await sendMail(email, `🚀 Activate Your Fero E-Library Account (${roleName})`, emailHtml);
    console.log(`   📧 Activation email sent successfully to ${email}!`);
  } catch (err) {
    console.error(`   ❌ Failed to onboard ${email}:`, err.message);
  }
}

onboardSingleEmail('opemipobayode21@gmail.com');
