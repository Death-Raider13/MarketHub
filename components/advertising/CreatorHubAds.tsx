...
import { getCreatorHubCampaigns, trackCampaignImpression, trackCampaignClick, DisplayCampaign } from "@/lib/advertising/campaign-display"
import Image from "next/image"

interface CreatorHubAdsProps {
  creatorId: string
...
export function CreatorHubAds({ 
  ...
  const loadCampaigns = async () => {
    try {
      setLoading(true)
      const activeCampaigns = await getCreatorHubCampaigns(creatorId, maxAds)
      setCampaigns(activeCampaigns)
    } catch (error) {
      console.error("Error loading creator hub campaigns:", error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const trackImpression = async (campaign: DisplayCampaign) => {
    try {
      await trackCampaignImpression(campaign.id, {
        placement: `creator_hub_${placement}`,
        creatorId,
